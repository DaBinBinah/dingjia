...
> 详细案例复盘与防护铁律见专门文档：[docs/lessons-learned.md](file:///Users/yedabin/Desktop/PY/同花顺/docs/lessons-learned.md)

| # | 故障 / 隐蔽 Bug 案例 | 根因与规避规范 |
|---|---|---|
| 1 | **新增标的显示“暂无行情”** | 写入标的记录时未拉取行情，且非交易时段定时器跳过。**铁律**：`ths-create-watch` 与批量导入在入库前必须主动调用 `fetchQuotes` 获取最新行情，做到“添加即所得”。 |
| 2 | **云函数内部变量未定义崩溃** | `exports.main` 内部缺少 `today` 变量定义引发 `ReferenceError`。**铁律**：时间变量统一在入口初始化，部署前必须本地模拟执行一次。 |
| 3 | **盘中实时价格与历史日 K 冲突** | 盘中临时 K 线误写入历史收盘。**铁律**：盘中历史行情截止上个交易日，收盘后才写入当天最终收盘价。 |
| 4 | **近 20 日涨跌天数缺失** | 忽略了持平（`pct === 0`）天数。**铁律**：区间统计必须满足 `全集 = 上涨 + 下跌 + 持平`。 |
| 5 | **ETF 无分红方案时误显示 0%** | 空值被强转为 0。**铁律**：无方案即保持 `null` 并纯净隐藏，绝不显示虚假的 0.00% 或 ¥0.00。 |
| 6 | **同环境静态托管误覆盖** | 同环境存在根目录 `/`（亲子艺术）。**铁律**：静态托管发布必须指定独占子路径 `npx tcb hosting deploy ./web ths -e REDACTED_CLOUDBASE_ENV_ID`。 |
| 7 | **CDN 静态缓存生效延迟** | 前端资源必须靠 `?v=` 版本号，改动后必须统一升级版本后缀。 |
